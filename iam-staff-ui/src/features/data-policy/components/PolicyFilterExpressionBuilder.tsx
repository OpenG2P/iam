...
function ConditionRow({
    ...
    return (
        <div className="grid grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1.2fr)_auto] gap-3 items-center rounded-[10px] border border-[#ED7C22]/30 bg-[#F3F1E4] px-4 py-2">
            <FilterSelect
            ...
            <button
                ...
                onClick={onRemove}
                disabled={disabled}
                className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[10px] text-[#A1A1A1] hover:bg-[#F3F1E4] hover:text-red-500 disabled:opacity-50"
                title={t('remove')}
            >
                <X size={18} />
            </button>
...
function GroupEditor({
    ...
    return (
        <div
            className={`space-y-3 ${depth > 0 ? 'ml-3 border-l-2 border-[#ED7C22]/40 pl-3' : ''}`}
        >
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-[10rem] max-w-xs shrink-0">
                    <label className="block text-[16px] font-medium text-[#000000] truncate">
                    ...
                    <p className="mt-1 text-sm text-[#A1A1A1] leading-snug">
                        {group.operator === 'AND'
                ...
                <div className="flex flex-wrap items-center justify-end gap-x-3 gap-y-1 shrink-0 ml-auto">
                    <button
                        ...
                        className="text-sm font-semibold text-[#ED7C22] hover:underline disabled:opacity-50"
                    >
                        + {t('condition')}
                    </button>
                    {group.allowNestedGroups !== false && (
                        <button
                            ...
                            className="text-sm font-semibold text-[#000000]/70 hover:underline disabled:opacity-50"
                        >
                            + {t('group')}
                        </button>
                    )}
                    ...
                    {onReset && (
                        <button
                            ...
                            onClick={onReset}
                            disabled={disabled}
                            className="text-sm font-semibold text-[#A1A1A1] hover:underline disabled:opacity-50"
                        >
                            {t('reset_filter')}
                        </button>
                    )}
